...
import { Menu, X, Plus, MessageCircle, Settings, Upload, Send, User, FileText, Clock, MoreHorizontal, ZoomIn, ZoomOut, RotateCw, Download, Trash2, AlertCircle, LogOut, Zap } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const [suggestions] = useState<Suggestion[]>([
    {
        id: "1",
        text: "Generate 5 exam-style questions with detailed answers based on the content of the uploaded PDF"
    },
    {
        id: "2",
        text: "Create multiple-choice, short-answer, and descriptive questions from the key concepts in the PDF"
    },
    {
        id: "3",
        text: "Extract questions that cover definitions, methodologies, results, and conclusions from the PDF"
    },
    {
        id: "4",
        text: "Formulate academic questions that test understanding of the main arguments and supporting evidence in the PDF"
    }
  ]);
  ...
  const renderPDFViewer = () => {
    return (
      <div className="w-1/2 bg-gradient-to-br from-green-50 to-white flex flex-col border-r border-green-200">
        {/* PDF Header */}
        <div className="bg-white border-b border-green-200 px-4 py-3 shadow-sm">
            ...
            <div className="flex items-center space-x-2">
              {pdfUrl && (
                <>
                  ...
                  <span className="text-sm text-green-700 font-medium px-2">{Math.round(pdfZoom * 100)}%</span>
                  <button
                  ...
                  <button
                      ...
                      className="bg-gradient-to-r from-green-600 to-green-700 text-white px-4 py-2 rounded-lg hover:from-green-700 hover:to-green-800 transition-all duration-200 text-sm shadow-md hover:shadow-lg transform hover:scale-105"
                    >
              ...
              <button
                ...
                className="bg-gradient-to-r from-green-600 to-green-700 text-white px-3 py-2 rounded-lg hover:from-green-700 hover:to-green-800 transition-all duration-200 flex items-center space-x-2 text-sm shadow-md hover:shadow-lg transform hover:scale-105"
          ...
          {isUploading && (
            <div className="mt-2">
              ...
              <p className="text-xs text-green-700 mt-1">{uploadProgress}% uploaded</p>
            </div>
          )}
        </div>

        {/* PDF Content */}
        <div className="flex-1 overflow-auto bg-gradient-to-br from-green-50 to-white p-4">
          {pdfUrl ? (
            <div className="h-full bg-white rounded-lg shadow-lg border border-green-200 overflow-hidden">
              <iframe
                src={pdfUrl}
            ...
            <div className="h-full bg-white rounded-lg shadow-lg border border-green-200 flex items-center justify-center">
              <div className="text-center text-green-700">
                <FileText className="w-16 h-16 mx-auto mb-4 text-green-400" />
                <p className="text-lg font-medium mb-2 text-green-800">No PDF uploaded</p>
                <p className="text-sm mb-4 text-green-600">Upload a PDF to start analyzing</p>
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="bg-gradient-to-r from-green-600 to-green-700 text-white px-4 py-2 rounded-lg hover:from-green-700 hover:to-green-800 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
                  disabled={isUploading}
                >
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="flex items-center space-x-2 px-4 py-2 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition-all duration-200"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {isLoadingProfile ? (
                    <div className="animate-pulse">
                      <div className="flex items-center space-x-4 mb-6">
                        ...
                        <div>
                          <div className="h-6 bg-green-300 rounded w-32 mb-2"></div>
                          <div className="h-4 bg-green-300 rounded w-48"></div>
                        </div>
                      </div>
                    </div>
                  ) : userProfile ? (
                    <>
                      <div className="flex items-center space-x-4 mb-6">
                        ...
                        <div>
                          <h3 className="text-xl font-semibold text-green-800">{userProfile.full_name}</h3>
                          <p className="text-green-700">{userProfile.email}</p>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <input 
                            type="text" 
                            className="w-full p-3 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200" 
                            defaultValue={userProfile.full_name} 
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-green-700 mb-2">Username</label>
                          <input 
                            type="text" 
                            className="w-full p-3 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200" 
                        ...
                        <button className="bg-gradient-to-r from-green-600 to-green-700 text-white px-6 py-2 rounded-lg hover:from-green-700 hover:to-green-800 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105">
                          Save Changes
                        </button>
                      </div>
                    </>
                ...
                {/* Chat Settings */}
                <div className="bg-white rounded-lg shadow-md border border-green-200 p-6">
                  <h3 className="text-lg font-semibold text-green-800 mb-4">Chat Settings</h3>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <label className="text-sm font-medium text-green-700">Use Comprehensive Mode</label>
                      <input
                        type="checkbox"
                        checked={useComprehensive}
                        onChange={(e) => setUseComprehensive(e.target.checked)}
                        className="rounded focus:ring-green-500 focus:ring-2"
                      />
                    </div>
                ...
                {/* Files */}
                <div className="bg-white rounded-lg shadow-md border border-green-200 p-6 md:col-span-2">
                  <h3 className="text-lg font-semibold text-green-800 mb-4">Uploaded Files</h3>
                  {uploadedFiles.length === 0 ? (
                    <p className="text-green-600">No files uploaded yet.</p>
                  ) : (
          ...
          <div className="flex-1 flex">
            ...
            <div className={`${!isSidebarOpen ? 'w-1/2' : 'w-full'} flex flex-col bg-white`}>
              ...
              {chatHistory.length === 0 && !isLoading && (
                <div className="flex-1 flex items-center justify-center p-8 bg-gradient-to-br from-green-50 to-white">
                  <div className="text-center max-w-2xl">
                    ...
                    {currentFile && (
                      <div className="space-y-3 mb-6">
                        <p className="text-sm font-medium text-green-700">Try asking:</p>
                        <div className="grid gap-2">
                          {suggestions.map((suggestion) => (
                            <button
                              key={suggestion.id}
                              onClick={() => handleSuggestionClick(suggestion.text)}
                              className="p-3 text-left bg-white hover:bg-green-50 rounded-lg border border-green-200 transition-all duration-200 hover:shadow-md"
                            >
                              <span className="text-sm text-green-700">{suggestion.text}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  {isLoading && (
                    <div className="flex justify-start">
                      <div className="max-w-3xl px-4 py-3 rounded-lg bg-green-50 border border-green-200">
                        <div className="flex items-center space-x-3">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="rounded focus:ring-green-500 focus:ring-2"
                    />
                    <label htmlFor="comprehensive" className="text-sm text-green-700">
                      Comprehensive
                    </label>
                  </div>
                  ...
                  <div className="flex-1 relative">
                    <textarea
                      ...
                      className="w-full p-3 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent resize-none transition-all duration-200"
                      rows={1}
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-green-600 to-green-700 text-white p-3 rounded-lg hover:from-green-700 hover:to-green-800 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
                  >
  ...
  const renderSidebar = () => (
    <div className={`fixed top-0 left-0 z-50 w-80 h-screen bg-white shadow-2xl border-r-2 border-green-200 transform transition-all duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0 opacity-100 scale-100' : '-translate-x-full opacity-0 scale-95'} flex flex-col`}>
      {/* Sidebar Header */}
      <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-green-50 to-emerald-50">
          <div className="flex items-center space-x-3 group">
            <div className="relative">
              <div className="absolute inset-0 bg-green-500 rounded-lg blur-sm opacity-30 group-hover:opacity-50 transition-opacity duration-300"></div>
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 font-medium">AI Quest Generator</p>
            </div>
          </div>
          <button 
            onClick={() => setIsSidebarOpen(false)}
            className="group relative p-2 text-green-600 hover:text-white hover:bg-green-500 rounded-full transition-all duration-300 hover:scale-105 hover:shadow-md"
          >
            <div className="absolute inset-0 bg-green-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      ...
      <div className="p-6 flex-shrink-0">
        <button
          ...
          className="w-full bg-gradient-to-r from-green-600 to-green-700 text-white px-6 py-4 rounded-xl hover:from-green-700 hover:to-green-800 transition-all duration-200 flex items-center justify-center space-x-3 shadow-lg hover:shadow-xl transform hover:scale-105"
        >
        ...
        <nav className="space-y-3">
          <button
            ...
            className={`w-full flex items-center space-x-4 px-4 py-3 rounded-xl transition-all duration-200 ${
              currentView === 'chat' 
                ? 'bg-gradient-to-r from-green-100 to-green-50 text-green-800 shadow-md border border-green-200' 
      ...
      {/* User Profile Footer */}
      <div className="border-t border-green-100 p-6 bg-gradient-to-r from-green-50 to-white flex-shrink-0">
        {userProfile ? (
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-base font-semibold text-green-800 truncate">{userProfile.full_name}</p>
              <p className="text-sm text-green-600 truncate">{userProfile.email}</p>
            </div>
            <button
              onClick={() => {
                setCurrentView('settings');
              ...
              className="p-3 text-green-600 hover:text-green-800 hover:bg-green-100 rounded-full transition-all duration-200"
            >
